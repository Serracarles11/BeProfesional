create or replace function public.usuario_puede_ver_entrenamientos(_equipo_id uuid)
returns boolean
language sql
stable
as $$
...
drop policy if exists "leer entrenamientos si eres miembro activo del equipo" on public.entrenamientos_equipo;
create policy "leer entrenamientos si eres miembro activo del equipo"
on public.entrenamientos_equipo
for select
using (
  public.usuario_puede_ver_entrenamientos(equipo_id)
);
